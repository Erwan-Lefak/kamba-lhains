...
export default function Chemise() {
  ...
  // Filtrer les produits contenant "chemise" dans le nom ou la description
  const chemiseProducts = products.filter(product => {
    const nameMatch = product.name.toLowerCase().includes('chemise');
    const descriptionMatch = Array.isArray(product.description) 
      ? product.description.some(desc => 
          desc.toLowerCase().includes('chemise')
        )
      : product.description.toLowerCase().includes('chemise');
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Découvrez notre sélection de Chemises - Élégance et style sophistiqué." />
              ...
              <li>
                ...
                {showHautSubmenu && (
                  <ul className="submenu">
                    ...
                    <li><button onClick={() => router.push('/chemise')} className="submenu-item active">CHEMISE</button></li>
          ...
          <div className={styles.threeProductsGrid}>
            {chemiseProducts.length > 0 ? (
              ...
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
                <p>Aucune chemise disponible pour le moment.</p>
              </div>
            )}
          </div>
          
          {/* Mobile Carousel */}
          {chemiseProducts.length > 0 && <MobileCarousel products={chemiseProducts} />}
        </section>
        </div>
      </main>
