...
export default function CrepusculePantalonCargo() {
  ...
  const pantalonCargoProducts = products.filter(product => {
    const isCrepuscule = product.category === 'Crépuscule';
    const nameMatch = product.name.toLowerCase().includes('pantalon') || 
    ...
    const descriptionMatch = Array.isArray(product.description) 
      ? product.description.some(desc => 
          desc.toLowerCase().includes('pantalon') || 
          desc.toLowerCase().includes('cargo')
        )
      : product.description.toLowerCase().includes('pantalon') || 
        product.description.toLowerCase().includes('cargo');
  ...
  return (
    <>
      <Head>
        <title>Pantalon Cargo Crépuscule - Kamba Lhains</title>
        <meta name="description" content="Découvrez nos Pantalons Cargo de la collection Crépuscule - Mystère et élégance pour vos soirées sophistiquées." />
              ...
              {[
                ...
                <div key={index} className={styles.gallerySlot}>
                  <Image 
                    ...
                    alt={`Pantalon Cargo Crépuscule ${index + 1}`}
                    width={400}
                    height={600}
