...
export default function AubeSacDeSport() {
  ...
  useEffect(() => {
    ...
    const shuffled = shuffleArray(products.filter(p => p.subCategory === 'aube' && p.id !== 'aube-sac-de-sport-001' && p.inStock)).slice(0, 4);
  ...
  const handleHeartClick = () => {
    if (!sacDeSportProduct) return;

    if (isFavorite(sacDeSportProduct.id, selectedColor)) {
      removeFromFavorites(sacDeSportProduct.id, selectedColor);
    } else {
      addToFavorites(sacDeSportProduct, selectedColor, selectedSize || undefined);
    }
  };
          ...
          <div className={productStyles.productPage} style={{marginTop: 0}}>
            <div className={productStyles.productContainer}>
              ...
              <div className={productStyles.productImageSection} style={{position: 'relative'}}>
                ...
                <button
                  className={`${productStyles.heartIcon} ${sacDeSportProduct && isFavorite(sacDeSportProduct.id, selectedColor) ? productStyles.liked : ''}`}
                  onClick={handleHeartClick}
                  aria-label={sacDeSportProduct && isFavorite(sacDeSportProduct.id, selectedColor) ? "Retirer des favoris" : "Ajouter aux favoris"}
                  style={{
                    position: 'sticky',
                    top: '10px',
                    right: '10px',
                ...
                >
                  <span className={`u-w-full ${sacDeSportProduct && isFavorite(sacDeSportProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-add`}>
                    <svg className="c-icon" data-size="sm">
                      <use xlinkHref="#icon-heart-kamba-plain" x="0" y="0"></use>
                    </svg>
                  </span>
                  <span className={`u-w-full ${!sacDeSportProduct || !isFavorite(sacDeSportProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-remove`}>
                    <svg className="c-icon" data-size="sm">
              ...
              <div className={productStyles.productInfoSection}>
                ...
                <div className={productStyles.productMainContent}>
                  <h1 className={productStyles.productTitle}>{currentLanguage === 'en' && sacDeSportProduct.nameEn ? sacDeSportProduct.nameEn : sacDeSportProduct.name}</h1>
