...
export default function ZenithVeste() {
  ...
  useEffect(() => {
    ...
    const shuffled = shuffleArray(products.filter(p => p.subCategory === 'zenith' && p.id !== 'bombers-itoua-zenith')).slice(0, 4);
  ...
  // Product page functions - utiliser le produit bombers depuis products.ts
  const bombersProduct = products.find(p => p.id === 'bombers-itoua-zenith') || {
    id: 'bombers-itoua-zenith',
    name: 'BOMBERS ITOUA',
    nameEn: 'ITOUA BOMBER',
    price: 1120,
    image: '/images/bombers-cafe-1.jpg?v=1',
    images: ['/images/bombers-cafe-1.jpg?v=1'],
    category: 'homme',
    subCategory: 'zenith',
    description: ['Bomber extra oversize'],
    colors: ['Café', 'Gris'],
  ...
  const handleAddToCart = () => {
    addToCart(bombersProduct, selectedSize, selectedColor, quantity);
  };

  const handleHeartClick = () => {
    if (!bombersProduct) return;

    if (isFavorite(bombersProduct.id, selectedColor)) {
      removeFromFavorites(bombersProduct.id, selectedColor);
    } else {
      addToFavorites(bombersProduct, selectedColor, selectedSize || undefined);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>Veste - Kamba Lhains</title>
            ...
            <div className={styles.mediaSection} style={{ width: '100%' }}>
              <div className="zenith-images-container">
                <div className="zenith-image-wrapper">
                  <Image
                    src="/images/zenith-veste-hero.jpg?v=2"
            ...
            <div className={productStyles.productContainer}>
              ...
              <div className={productStyles.productImageSection} style={{position: 'relative'}}>
                ...
                <div className={productStyles.breadcrumb} style={{
                  ...
                  <span> - </span>
                  <Link href="/zenith/veste" className={productStyles.breadcrumbLink}>
                    <span>{t('products.breadcrumb.veste')}</span>
                  </Link>
                  <span> - </span>
                  <Link href="/zenith/veste" className={productStyles.breadcrumbLink}>
                    <span>Bombers</span>
                ...
                <button
                  className={`${productStyles.heartIcon} ${bombersProduct && isFavorite(bombersProduct.id, selectedColor) ? productStyles.liked : ''}`}
                  onClick={handleHeartClick}
                  aria-label={bombersProduct && isFavorite(bombersProduct.id, selectedColor) ? t('products.removeFromFavorites') : t('products.addToFavorites')}
                  style={{
                    position: 'sticky',
                    top: '10px',
                    right: '10px',
                ...
                >
                  <span className={`u-w-full ${bombersProduct && isFavorite(bombersProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-add`}>
                    <svg className="c-icon" data-size="sm">
                      <use xlinkHref="#icon-heart-kamba-plain" x="0" y="0"></use>
                    </svg>
                  </span>
                  <span className={`u-w-full ${!bombersProduct || !isFavorite(bombersProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-remove`}>
                    <svg className="c-icon" data-size="sm">
                ...
                <div className={productStyles.imageStack}>
                  {bombersProduct.images?.map((image, index) => {
                    const imageUrl = typeof image === 'string' ? image : image.url;
                    return (
                    <Image
                      key={index}
                      src={imageUrl}
                      alt={`${bombersProduct.name} ${index + 1}`}
                      width={600}
                      height={750}
              ...
              <div className={productStyles.productInfoSection}>
                ...
                <div className={productStyles.productMainContent}>
                  <h1 className={productStyles.productTitle}>{currentLanguage === 'en' && bombersProduct.nameEn ? bombersProduct.nameEn : bombersProduct.name}</h1>
                  <span className={productStyles.productPrice}>{bombersProduct.price} EUR</span>
                  ...
                  <div className={productStyles.colorSection}>
                    ...
                    <div className={productStyles.colorOptions}>
                      {bombersProduct.colors?.map((color, index) => {
                  ...
                  <div className={productStyles.sizeSection}>
                    ...
                    <div className={productStyles.sizeGrid}>
                      {bombersProduct.sizes?.map((size, index) => (
                        <button
                          key={index}
