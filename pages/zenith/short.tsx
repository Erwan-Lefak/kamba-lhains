...
export default function ZenithShort() {
  ...
  useEffect(() => {
    ...
    const shuffled = shuffleArray(products.filter(p => p.subCategory === 'zenith' && p.id !== '17')).slice(0, 4);
  ...
  const handleHeartClick = () => {
    if (!shortProduct) return;

    if (isFavorite(shortProduct.id, selectedColor)) {
      removeFromFavorites(shortProduct.id, selectedColor);
    } else {
      addToFavorites(shortProduct, selectedColor, selectedSize || undefined);
    }
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={t('meta.short')} />
            ...
            <div className={productStyles.productContainer}>
              ...
              <div className={productStyles.productImageSection} style={{position: 'relative'}}>
                ...
                <div className={productStyles.breadcrumb} style={{
                  ...
                  <span> - </span>
                  <Link href="/zenith/short" className={productStyles.breadcrumbLink}>
                    <span>{t('products.breadcrumb.short')}</span>
                  </Link>
                </div>

                {/* Heart Icon */}
                <button
                  className={`${productStyles.heartIcon} ${shortProduct && isFavorite(shortProduct.id, selectedColor) ? productStyles.liked : ''}`}
                  onClick={handleHeartClick}
                  aria-label={shortProduct && isFavorite(shortProduct.id, selectedColor) ? t('products.removeFromFavorites') : t('products.addToFavorites')}
                  style={{
                    position: 'sticky',
                    top: '10px',
                    right: '10px',
                ...
                >
                  <span className={`u-w-full ${shortProduct && isFavorite(shortProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-add`}>
                    <svg className="c-icon" data-size="sm">
                      <use xlinkHref="#icon-heart-kamba-plain" x="0" y="0"></use>
                    </svg>
                  </span>
                  <span className={`u-w-full ${!shortProduct || !isFavorite(shortProduct.id, selectedColor) ? 'u-hidden' : ''} | js-product-heart-remove`}>
                    <svg className="c-icon" data-size="sm">
              ...
              <div className={productStyles.productInfoSection}>
                <div className={productStyles.productMainContent}>
                  <h1 className={productStyles.productTitle}>{currentLanguage === 'en' && shortProduct.nameEn ? shortProduct.nameEn : shortProduct.name}</h1>
