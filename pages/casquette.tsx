...
export default function Casquette() {
  ...
  // Filtrer les produits contenant "casquette" ou "cap" dans le nom ou la description
  const casquetteProducts = products.filter(product => {
    const nameMatch = product.name.toLowerCase().includes('casquette') || 
    ...
    const descriptionMatch = Array.isArray(product.description) 
      ? product.description.some(desc => 
          desc.toLowerCase().includes('casquette') || 
          desc.toLowerCase().includes('cap')
        )
      : product.description.toLowerCase().includes('casquette') || 
        product.description.toLowerCase().includes('cap');
  ...
  return (
    <>
      <Head>
        <title>Casquette - Kamba Lhains</title>
        <meta name="description" content="Découvrez notre sélection de Casquettes - Style sportif et protection solaire." />
      ...
      <main className="kambavers-page">
              ...
              <li>
                ...
                {showAccessoiresSubmenu && (
                  <ul className="submenu">
                    ...
                    <li><button onClick={() => router.push('/casquette')} className="submenu-item active">CASQUETTE</button></li>
        ...
        <div className={`main-content ${isMenuVisible ? 'with-sidebar' : 'full-width'}`}>
          {/* Section Introduction Casquette */}
          <section className={styles.newCollectionSection}>
            <div className={styles.textSection}>
              ...
              <p className={styles.collectionDescription}>
                Découvrez notre collection de casquettes, accessoires incontournables du style sportif et de la protection solaire. Des designs urbains et des coupes ajustées pour parfaire vos looks décontractés avec authenticité.
              </p>
            </div>
        ...
        <section className={styles.threeProductsSection}>
          ...
          <div className={styles.threeProductsGrid}>
            {casquetteProducts.length > 0 ? (
              casquetteProducts.map(product => (
          ...
          {/* Mobile Carousel */}
          {casquetteProducts.length > 0 && <MobileCarousel products={casquetteProducts} />}
        </section>
        </div>
      </main>
