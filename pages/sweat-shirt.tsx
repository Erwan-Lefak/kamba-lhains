...
export default function SweatShirt() {
  ...
  // Filtrer les produits contenant "sweat" dans le nom ou la description
  const sweatProducts = products.filter(product => {
    const nameMatch = product.name.toLowerCase().includes('sweat');
    const descriptionMatch = Array.isArray(product.description) 
      ? product.description.some(desc => 
          desc.toLowerCase().includes('sweat')
        )
      : product.description.toLowerCase().includes('sweat');
  ...
  return (
    <>
      <Head>
        <title>Sweat-Shirt - Kamba Lhains</title>
        <meta name="description" content="Découvrez notre sélection de Sweat-Shirts - Confort moderne et style décontracté." />
      ...
      <main className="kambavers-page">
              ...
              <li>
                ...
                {showHautSubmenu && (
                  <ul className="submenu">
                    ...
                    <li><button onClick={() => router.push('/sweat-shirt')} className="submenu-item active">SWEAT-SHIRT</button></li>
        ...
        <div className={`main-content ${isMenuVisible ? 'with-sidebar' : 'full-width'}`}>
          {/* Section Introduction Sweat-Shirt */}
          <section className={styles.newCollectionSection}>
            <div className={styles.textSection}>
              ...
              <p className={styles.collectionDescription}>
                Explorez notre gamme de sweat-shirts, parfait équilibre entre confort moderne et style décontracté. Des pièces essentielles qui allient douceur, praticité et esthétique urbaine pour un look relaxé et tendance.
              </p>
            </div>
        ...
        <section className={styles.threeProductsSection}>
          ...
          <div className={styles.threeProductsGrid}>
            {sweatProducts.length > 0 ? (
              sweatProducts.map(product => (
              ...
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
                <p>Aucun sweat-shirt disponible pour le moment.</p>
              </div>
            )}
          </div>
          
          {/* Mobile Carousel */}
          {sweatProducts.length > 0 && <MobileCarousel products={sweatProducts} />}
        </section>
        </div>
      </main>
