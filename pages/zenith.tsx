...
export default function Zenith() {
  ...
  const zenithProducts = products.filter(product => {
    return product.subCategory === 'zenith';
  });

  return (
    <>
      <Head>
        <title>{t('zenith.metaTitle')}</title>
        <meta name="description" content={t('zenith.metaDescription')} />
      ...
      <main className="kambavers-page">
          ...
          <section className={styles.newCollectionSection}>
            ...
            <div className={styles.mediaSection}>
              <div className={styles.imageContainer}>
                <Image
                  src="/images/zenith-hero.jpg?v=2"
          ...
          <section className={styles.twoProductsSection}>
            <div className={styles.twoProductsGrid}>
              <div className={styles.simpleProductSlot}>
                <Image
                  src="/images/zenith-section2.jpg?v=2"
                  alt="Collection Zénith"
                  width={1200}
                  height={800}
                  className={styles.collectionImage}
                  quality={95}
              ...
              <div className={styles.textZone}>
                <p className={styles.textZoneContent}>
                  {t('zenith.section2Text').split('\n\n').map((paragraph: string, index: number) => (
                    <React.Fragment key={index}>
                      {paragraph}
                      {index < t('zenith.section2Text').split('\n\n').length - 1 && <><br /><br /></>}
                    </React.Fragment>
                  ))}
                </p>
              </div>
        ...
        <section className={styles.threeProductsSection}>
          ...
          <div className={styles.threeProductsGrid}>
            {zenithProducts.map(product => (
              <div key={product.id} className={styles.productSlot}>
                <ProductCard
                  product={product}
                  defaultColor={product.id === '7' ? '#22C55E' : undefined}
                />
              </div>
            ))}
          </div>
          ...
          {/* Mobile Carousel */}
          <MobileCarousel
            products={zenithProducts}
            defaultColorByProductId={{ '7': '#22C55E' }}
          />
        </section>
        </div>
      </main>
